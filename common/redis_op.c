 ...
 */
/* -------------------------------------------*/
int rop_set_key_expire(redisContext *conn, char *key, unsigned int seconds)
{
	int retn = 0;
	redisReply *reply = NULL;		
	
	reply = redisCommand(conn, "EXPIRE %s %d", key, seconds);
	if (reply->type != REDIS_REPLY_INTEGER) {
		fprintf(stderr, "[-][GMS_REDIS]Set key:%s expire ERROR!\n", key);
		LOG(REDIS_LOG_MODULE, REDIS_LOG_PROC, "[-][GMS_REDIS]Set key:%s expire ERROR! %s\n", key, conn->errstr);
		retn = -1;
	}
	if (reply->integer == 1) {
		/* 成功 */
...
int rop_set_string(redisContext *conn, char *key, char *value)
{
	...
	if(NULL == conn){
		LOG(REDIS_LOG_MODULE, REDIS_LOG_PROC, "rop_set_string error redis 初始化错误,请连接redis服务器后再试\n");
		retn = -1;
        goto END;
	}
	...
	if(NULL == reply){
		LOG(REDIS_LOG_MODULE, REDIS_LOG_PROC, "rop_set_string error set %s %s\n", key,value);
		retn = -1;
        goto END;
	}
